...
def main():
    ...
    city_num = 10
    population_size = 5 #Ensure population size is a MULTIPLE of 5
    generations = 10
    mutation_rate = 0.3
    ...
    for i in range(0, len(population)):
        if (random.uniform(0, 1) < mutation_rate):
            ...
            for j in range(0, reps):
                while True:
                    a = random.randrange(len(population[i].order)-1)
                    b = random.randrange(len(population[i].order)-1)
                    if (a != b):
                        break
